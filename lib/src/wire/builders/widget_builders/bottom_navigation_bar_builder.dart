...
class BottomNavigationBarBuilder extends WidgetBuilder {
  ...
  material.Widget build(Json json, material.BuildContext? context) {
    ...
    return material.BottomNavigationBar(
        items: List<material.BottomNavigationBarItem>.from(json.data["items"]
            .map((x) => material.BottomNavigationBarItem(
                icon: application
                    ...
                    .build(Json.fromJson(x["data"]["icon"]), context),
                label: x["data"]["label"]))),
        currentIndex: application
            .make<JsonBuilder<int>>(json.data["currentIndex"]["type"])
            .build(Json.fromJson(json.data["currentIndex"]["data"]), context),
        ...
        selectedFontSize:
            json.data["selectedFontSize"] == null ? 14.0 : json.data["selectedFontSize"].toDouble(),
