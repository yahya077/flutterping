...
class PreLoader extends Loader {
  ...
  Future<T> load<T>(String loaderKey) async {
    final config_model.WireConfig config = application.make<WireConfig>(WireDefinition.config).allAs();
    ...
    final response = await application.make<Client>(loader.client).request(loader.endpoint, method: loader.method);
